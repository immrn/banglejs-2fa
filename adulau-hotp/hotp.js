...
var JSSHAsha1 = require("sha1.js");
var Crypto = require('cryptojs-ru-hmac-sha1.js');
...
function hotp(key, counter, format) {
    ...
    console.log("hotp_movingfactortohex(counter) =", hotp_movingfactortohex(counter));
    console.log("hotp_hexkeytobytestream(key) =", hotp_hexkeytobytestream(key));
    ...
    /*
    var hmacBytes = JSSHAhmac(hexStrToBytes(hotp_hexkeytobytestreamkey), hexStrToBytes(hotp_movingfactortohexcounter), keyT, counterT)

    if (format == "hex40") {
        return hmacBytes.substring(0, 10);
    } else if (format == "dec6") {
        return truncatedvalue(hexStrToBytes(hmacBytes), 6);
    } else if (format == "dec7") {
        return truncatedvalue(hexStrToBytes(hmacBytes), 7);
    } else if (format == "dec8") {
        return truncatedvalue(hexStrToBytes(hmacBytes), 8);
    }
    else {
